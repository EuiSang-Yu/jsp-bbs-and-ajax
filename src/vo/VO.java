...
public class VO {
	...
	public static final String SQL_WRITE_INSERT = "INSERT INTO tb_board"
			+ "(board_id, board_title, board_content, board_regDate, board_viewCnt, board_likeCnt, board_replyCnt, board_champion) "
			+ "VALUES" + "(SEQ_tb_board_board_id.NEXTVAL, ?, ?, SYSDATE, 0, 0, 0, ?)"; // 챔피언 넘버 추가

	public static final String SQL_WRITE_SELECT = 
			
			"SELECT * FROM tb_board WHERE board_champion = ? ORDER BY board_id DESC"; // 챔피언
	...
	public static final String SQL_WRITE_INC_VIEWCNT =
			...
			"UPDATE tb_board SET board_viewCnt = board_viewCnt + 1 WHERE board_id = ? AND board_champion=?"; // 챔피언 넘버
																												// 추가
	public static final String SQL_USER_SIGNUP = "INSERT INTO TB_USER"
			+ "(user_uid, user_id, user_pw, user_name, user_email, user_phone) " + "VALUES"
			+ "(SEQ_tb_user_user_uid.NEXTVAL, ?, ?, ?, ?, ?)";

	public static final String SQL_WRITE_UPDATE = 
			
			"UPDATE TB_BOARD SET board_title = ?, board_content = ? WHERE board_id = ? and board_champion=?"; // 챔피언넘버추가
